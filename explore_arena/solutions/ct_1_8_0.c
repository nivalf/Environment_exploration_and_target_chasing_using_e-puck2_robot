...
#define INNER_PROX_THR (3 * TARGET_PROX_THR)
#define OUTER_PROX_THR (0.4 * TARGET_PROX_THR)
...
void set_turn_direction(void) {
	...
	int direction = right_sum > left_sum ? 1 : -1;
...
int target_in_proximity(void) {
	...
	const int thr1 = 0.7*INNER_PROX_THR;
	...
	if(bot_state == 1) {			// moving forward
		...
		in_proximity = (get_prox(0)<thr1) && (get_prox(7)<thr1);
	}
