 ...
 *
 * Fix:
 * 1. move back state added
 * - If target moves forward to the bot while bot is at standby mode, move back
 *
 * Known issues:
...
int target_in_proximity_backwards(void);
...
void should_stop_move_back(void) {
	if(target_in_proximity_backwards()) {
		bot_state = 2;
...
/* Check if bot is in the proximity of the target: while moving forward
 *
 * return:
...
/* Check if bot is in the proximity of the target: while moving backwards
 *
 * return:
 * 	1 : target in proximity
 *  0 : target not in proximity
...
int target_moved_out_of_proximity(void) {
	...
	const int thr = 0.7 * TARGET_PROX_THR;
...
int target_too_close(void) {
	...
	// sensors register target at max 150% proximity threshold
	const int thr = 1.5 * TARGET_PROX_THR;
