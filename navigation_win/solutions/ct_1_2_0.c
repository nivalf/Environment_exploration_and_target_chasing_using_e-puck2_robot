 ...
 * => If target reached, go to standby state.
 *
 * Problems:
 * 1. Additional exit required from move forward state
 * => Should exit if target not in range too
 *
 * 2. No exit from standby state
...
/* Switch bot state to standby mode if obstacle ahead*/
void should_stop_move_forward(void) {
	if(target_reached()) {
		bot_state = 2;
